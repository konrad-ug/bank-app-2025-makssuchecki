...
class TestCompanyAccount:
    ...
    def test_company_account_creation(self, mocker: MockFixture):
        ...
        assert company_account.company_name == "Tech Solutions"
        assert company_account.balance == 0.0
        assert company_account.nip == "8461627562"

    def test_company_account_raises_error_when_not_registered(self, mocker: MockFixture):
        mock = mocker.patch("src.company_account.requests.get")
        mock.return_value.status_code = 200
        ...
        with pytest.raises(ValueError, match="Company not registered"):
            CompanyAccount("Fake Corp", "8461627562")